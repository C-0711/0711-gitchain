...
# Ensure contributors exist (with correct schema)
contributors = [
    ...
    ('bombas@0711.io', 'Bombas', 'ai_agent', '0711 Intelligence')
]

for cid, name, role, org in contributors:
    cur.execute("""
        INSERT INTO contributors (id, name, role, organization)
        VALUES (%s, %s, %s, %s)
...
for layer_id, name, layer_type, contributor_id, trust in layers:
    ...
    """, (container_uuid, layer_id, name, layer_type, contributor_id, trust, 'import-test-001'))
...
field_names = {'breite': 'Breite', 'tiefe': 'Tiefe', 'hoehe': 'Höhe', 'gewicht': 'Gewicht'}
...
cur.execute("""
    SELECT cl.name, cl.atom_count, cl.trust_level
