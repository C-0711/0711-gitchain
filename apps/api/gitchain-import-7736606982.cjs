  ...
  try {
    ...
    for (const [category, specs] of Object.entries(specsByCategory)) {
      ...
      for (const spec of specs) {
        // Skip empty values
        if (!spec.value || spec.value.trim() === '') continue;
    ...
    // Add commit record (without author_id - it's a UUID FK)
    await client.query(`
      INSERT INTO container_commits (container_id, version, commit_hash, message, data, created_at)
      SELECT id, version, $2, 'Real Bosch data import: 369 specifications from datasheets', '{"author": "bombas@0711.io"}'::jsonb, NOW()
